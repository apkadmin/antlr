...
import CustomLangParser from "./CustomLangParser";

export default abstract class CustomLangLexerBase extends Lexer {
    tokens: any[];
    indents: any[];
    opened: number;
   ...
   override emitToken(token: Token) {
   ...
   override nextToken() {
        ...
        if (this._input.LA(1) === CustomLangParser.EOF && this.indents.length) {
            // Remove any trailing EOF tokens from our buffer.
            this.tokens = this.tokens.filter(function (val) {
                return val.type !== CustomLangParser.EOF;
            });
            // First emit an extra line break that serves as the end of the statement.
            this.emitToken(this.commonToken(CustomLangParser.NEWLINE, "\n"));
            ...
            this.emitToken(this.commonToken(CustomLangParser.EOF, "<EOF>"));
        }
    ...
    createDedent() {
        return this.commonToken(CustomLangParser.DEDENT, "");
    }
    ...
    onNewLine() {
        ...
        if (this.opened > 0 || (nextnext != -1 /* EOF */ && (next === 13 /* '\r' */ || next === 10 /* '\n' */ || next === 35 /* '#' */))) {
            ...
            this.emitToken(this.commonToken(CustomLangParser.NEWLINE, newLine));
            ...
            if (indent === previous) {
                ...
                this.emitToken(this.commonToken(CustomLangParser.INDENT, spaces));
            } else {
